...
class _MapScreenState extends State<MapScreen> {
  ...
  bool _refreshing = false; // фоновое обновление поверх кэша
  ...
  Future<void> _loadAll() async {
    ...
    if (cachedAgents != null && mounted) {
      setState(() {
        ...
        lineupCounts = counts;
        loading = false;
        _refreshing = true; // покажем индикатор фонового обновления
      });
    }

    // ── Шаг 2: свежие данные из API (с retry) ───────────────────────────────
    try {
      final freshAgents = await ValorantApi.getAgents();
      final visibleAgents = AgentsConfigService.applyDisabled(freshAgents, hidden);
      for (final agent in freshAgents) {
        final role = agent['role']?['displayName'] as String?;
        final name = agent['displayName'] as String?;
        debugPrint('AGENT: $name | ROLE: $role');
      }
      if (mounted) {
        setState(() {
        ...
        for (final agent in visibleAgents) {
          ...
          if (icon != null && icon.isNotEmpty) {
            precacheImage(CachedNetworkImageProvider(icon), context);
          }
          for (final ability in (agent['abilities'] as List? ?? [])) {
            ...
            if (abilIcon != null && abilIcon.isNotEmpty) {
              precacheImage(CachedNetworkImageProvider(abilIcon), context);
            }
          }
        }
      }
      ...
      if (!mounted) return;
      if (agents.isEmpty) {
        // Нет ни кэша, ни сети — показываем экран ошибки
        setState(() {
          loading = false;
          _hasError = true;
          _refreshing = false;
        ...
        // Есть кэш — молча оставляем его, убираем индикатор
