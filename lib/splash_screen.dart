...
class _SplashScreenState extends State<SplashScreen> {
  ...
  Future<void> _initialize() async {
    ...
    try {
      ...
      await _advance(0.30);
      final updateCheck = await ForceUpdateService.checkUpdate();
      ...
      await _advance(0.35);
      final savedThemeType = await AppThemes.loadSaved();
      ...
      if (uid != null) {
        await AuthService.syncUsername();
        await AuthService.syncUserLevel();
    ...
    try {
      ...
      for (final agent in agents) {
        ...
        if (icon != null && icon.isNotEmpty) {
          precacheImage(CachedNetworkImageProvider(icon), context).ignore();
        }
        for (final ability in (agent['abilities'] as List? ?? [])) {
          ...
          if (abilIcon != null && abilIcon.isNotEmpty) {
            precacheImage(CachedNetworkImageProvider(abilIcon), context).ignore();
          }
        }
      }
    } catch (_) {}
