...
class AuthService {
  ...
  static Future<void> incrementApprovedLineups(String uid) async {
    ...
    try {
      ...
      final approved = (doc.data()?['approved_lineups'] as num?)?.toInt() ?? 0;
      final level = LevelSystem.getLevel(approved)['level'] as int;
      await _db.collection('users').doc(uid).update({'level': level});
    } catch (_) {}
  }
  ...
  static Future<void> syncUserLevel() async {
    if (userId == null) return;
    try {
      final doc = await _db.collection('users').doc(userId).get();
      if (!doc.exists) return;
      final approved = (doc.data()?['approved_lineups'] as num?)?.toInt() ?? 0;
      final prefs = await SharedPreferences.getInstance();
      await prefs.setInt('approved_lineups', approved);
      final level = LevelSystem.getLevel(approved)['level'] as int;
      final storedLevel = (doc.data()?['level'] as num?)?.toInt() ?? 0;
      if (storedLevel != level) {
        await _db.collection('users').doc(userId).update({'level': level});
      }
    } catch (_) {}
  }
}
