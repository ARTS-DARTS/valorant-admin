...
class _DuelScreenState extends State<DuelScreen> with TickerProviderStateMixin {
  static const _red = Color(0xFFFF4655);
  static const _green = Color(0xFF22C55E);
  static const _origRatio = 1.6;
  ...
  double _correctedY(double fy, double containerW, double containerH) {
    final ify = (fy * _origRatio - (_origRatio - 1) / 2).clamp(0.0, 1.0);
    final imgSize = math.min(containerW, containerH);
    final letterboxY = (containerH - imgSize) / 2;
    return letterboxY + ify * imgSize;
  }
      ...
      child: abilityIcon != null
          ? ClipOval(
              child: CachedNetworkImage(
                ...
                errorWidget: (_, _, _) => const Icon(Icons.circle,
                    color: _red, size: 16),
              ),
            )
          : const Icon(Icons.my_location, color: _red, size: 16),
    );
  }
...
class _TrajectoryPainter extends CustomPainter {
  ...
  final Color color;
  final double origRatio;

  const _TrajectoryPainter({
    required this.points,
    required this.progress,
    required this.color,
    required this.origRatio,
  });

  Offset _correctedOffset(Map<String, dynamic> p, Size size) {
    final fx = (p['x'] as num).toDouble();
    final fy = (p['y'] as num).toDouble();
    final ify = (fy * origRatio - (origRatio - 1) / 2).clamp(0.0, 1.0);
    final imgSize = math.min(size.width, size.height);
    final letterboxY = (size.height - imgSize) / 2;
    return Offset(fx * size.width, letterboxY + ify * imgSize);
  }

  // Catmull-Rom сплайн: делит каждый сегмент на [steps] частей
  static List<Offset> _catmullRom(List<Offset> pts, int steps) {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final rawPts = points.map((p) => _correctedOffset(p, size)).toList();
    final pts = _catmullRom(rawPts, 16); // плавная кривая
    ...
    // Точки только на исходных контрольных точках
    for (int i = 0; i < rawPts.length; i++) {
      final rawProgress = i / (rawPts.length - 1);
      if (rawProgress * totalLen <= targetLen + 0.1) {
        canvas.drawCircle(rawPts[i], 3.5, fillPaint);
      }
    }

    if ((currentEnd - prevEnd).distance > 1) {
      _drawArrow(canvas, prevEnd, currentEnd, fillPaint);
    }
  }
