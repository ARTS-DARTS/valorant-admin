...
class _GuideScreenState extends State<GuideScreen> with SingleTickerProviderStateMixin, WidgetsBindingObserver {
  // Плеер
  ...
  Future<void> _logLineupView() async {
    ...
    debugPrint('[Analytics] _logLineupView lineupId=${widget.lineupId} uid=$uid');

    // views_count можно инкрементить всегда (правило allow update: if isAuth() — но анон не может)
    // analytics_events требует авторизацию
    final futures = <Future>[
      db.collection('lineups').doc(widget.lineupId).update({
        'views_count': FieldValue.increment(1),
      }).then((_) {
        debugPrint('[Analytics] views_count incremented OK');
      }).catchError((Object e) {
        debugPrint('[Analytics] views_count FAILED: $e');
      }),
    ];
    ...
    if (uid != null) {
      futures.add(
        db.collection('analytics_events').add({
          ...
          debugPrint('[Analytics] analytics_events.lineup_viewed written OK');
        }).catchError((Object e) {
          debugPrint('[Analytics] analytics_events.lineup_viewed FAILED: $e');
        }),
      );
    } else {
      debugPrint('[Analytics] analytics_events skipped — user not authenticated');
    }
                      ...
                      itemBuilder: (context, i) => GestureDetector(
                        ...
                        child: ClipRRect(
                          ...
                          child: CachedNetworkImage(
                              imageUrl: widget.screenshots[i], fit: BoxFit.cover,
          ...
          if (widget.lineupId.isNotEmpty)
            Container(
              ...
              child: Row(
                ...
                children: [
                  StreamBuilder<bool>(
                    stream: LikesService.isLiked(widget.lineupId),
                    builder: (context, likedSnap) {
                      final liked = likedSnap.data ?? false;
                      return StreamBuilder<int>(
                        stream: LikesService.getLikesCount(widget.lineupId),
                        builder: (context, countSnap) {
                          final count = countSnap.data ?? 0;
                          return GestureDetector(
                            onTap: () async {
                              if (widget.isExclusive && _hasAccess != true) {
                                AppSnackBar.show(context, AppLocalizations.of(context)!.needExclusiveForLike, type: SnackBarType.warning);
                                return;
                              }
                            ...
                            child: AnimatedBuilder(
                              ...
                              child: Container(
                                ...
                                decoration: BoxDecoration(
                                  ...
                                  border: Border.all(color: liked ? t.primary : t.border),
                                ),
                                child: Row(
                                  mainAxisSize: MainAxisSize.min,
                                  children: [
                                    Icon(liked ? Icons.favorite : Icons.favorite_border,
                                        color: liked ? t.primary : t.textSecondary, size: 20),
                                    const SizedBox(width: 8),
                                    Text('$count',
                                        style: TextStyle(
