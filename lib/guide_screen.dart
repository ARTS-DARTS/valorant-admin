...
class _GuideScreenState extends State<GuideScreen> with WidgetsBindingObserver {
  ...
  // Watermark / сохранение в галерею
  String _username = 'Valorant Lineups';
  final GlobalKey _videoKey = GlobalKey();
  Offset _wmOffset = const Offset(20, 20);
  Offset _wmVelocity = const Offset(1.5, 1.2);
  Timer? _wmTimer;
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    } else if (state == AppLifecycleState.resumed && widget.isExclusive) {
      _checkAccess();
    }
  }
  ...
  Future<void> _castVote(bool isUp) async {
    ...
    final approved = (userDoc.data()?['approved_lineups'] as num?)?.toInt() ?? 0;
    final level = LevelSystem.getLevel(approved)['level'] as int;
    if (level < 2) {
  ...
  Future<void> _saveScreenshotWithWatermark(String imageUrl) async {
    try {
      ...
      final textPainter = TextPainter(
        text: TextSpan(text: 'Valorant Lineups\n@$_username', style: textStyle),
        textDirection: TextDirection.ltr,
      )..layout(maxWidth: size.width * 0.6);

      const padding = 20.0;
      final dx = size.width - textPainter.width - padding;
      final dy = size.height - textPainter.height - padding;

      canvas.drawRRect(
        RRect.fromRectAndRadius(
          Rect.fromLTWH(dx - 8, dy - 8, textPainter.width + 16, textPainter.height + 16),
      ...
      final img = await recorder.endRecording().toImage(srcImage.width, srcImage.height);
      final byteData = await img.toByteData(format: ui.ImageByteFormat.png);
      await Gal.putImageBytes(byteData!.buffer.asUint8List());

      if (mounted) AppSnackBar.show(context, 'Скриншот сохранён в галерею ✅', type: SnackBarType.success);
    } catch (_) {
      if (mounted) AppSnackBar.show(context, 'Ошибка сохранения', type: SnackBarType.error);
    }
  }

  void _startWatermarkBounce(Size containerSize) {
    _wmOffset = const Offset(20, 20);
    _wmVelocity = const Offset(1.5, 1.2);
    if (mounted) setState(() => _showWatermark = true);
    const wmSize = Size(200, 44);
    _wmTimer = Timer.periodic(const Duration(milliseconds: 16), (_) {
      if (!mounted) return;
      setState(() {
        _wmOffset += _wmVelocity;
        if (_wmOffset.dx <= 0 || _wmOffset.dx >= containerSize.width - wmSize.width) {
          _wmVelocity = Offset(-_wmVelocity.dx, _wmVelocity.dy);
        }
        if (_wmOffset.dy <= 0 || _wmOffset.dy >= containerSize.height - wmSize.height) {
          _wmVelocity = Offset(_wmVelocity.dx, -_wmVelocity.dy);
        }
      });
    });
  ...
  Future<void> _saveVideoToGallery() async {
    try {
      ...
      if (!await cacheFile.exists()) {
        if (mounted) AppSnackBar.show(context, 'Сначала сохрани оффлайн', type: SnackBarType.error);
        return;
      }
      await Gal.putVideo(cacheFile.path);
      if (mounted) AppSnackBar.show(context, 'Видео сохранено в галерею ✅', type: SnackBarType.success);
    } catch (_) {
      if (mounted) AppSnackBar.show(context, 'Ошибка сохранения видео', type: SnackBarType.error);
    }
  }
                            ...
                            children: [
                              ...
                              Row(
                                ...
                                children: [
                                  ...
                                  if (widget.authorId != null && widget.authorId!.isNotEmpty)
                                    UserBadgeRow(uid: widget.authorId!, size: 14),
                                ],
                              ),
                            ],
                          ),
                  ...
                  if (widget.screenshots.isNotEmpty) ...[
                    ...
                    GridView.builder(
                      ...
                      itemBuilder: (context, i) => GestureDetector(
                        onLongPress: () => _saveScreenshotWithWatermark(widget.screenshots[i]),
                        onTap: () {
                          _controller?.pause();
                          Navigator.push(
                            context,
                            ...
                            children: [
                              ...
                              Positioned(
                                ...
                                child: GestureDetector(
                                  onTap: () => _saveScreenshotWithWatermark(widget.screenshots[i]),
                                  child: Container(
                                    ...
                                    child: const Row(
                                      ...
                                      children: [
                                        ...
                                        SizedBox(width: 3),
                                        Text('Сохранить', style: TextStyle(color: Colors.white, fontSize: 10)),
                                      ],
                                    ),
                                  ),
                                ),
                            ...
                            builder: (context, constraints) {
                              ...
                              return Stack(
                                children: [
                                  ...
                                  // Невидимый слой для запуска bounce при просмотре (long press)
                                  Positioned.fill(
                                    child: GestureDetector(
                                      behavior: HitTestBehavior.translucent,
                                      onLongPress: () {
                                        if (_showWatermark) {
                                          _stopWatermarkBounce();
                                        } else {
                                          _startWatermarkBounce(size);
                                        }
                                      },
                                    ),
                                  ),
                ...
                if (_isCached) ...[
                  ...
                  SizedBox(
                    ...
                    child: TextButton.icon(
                      ...
                      label: const Text('Сохранить видео в галерею'),
...
class _VideoPlayerWidgetState extends State<_VideoPlayerWidget> {
  ...
  void _onUpdate() {
    ...
    if (!_seekPending && dur > Duration.zero && pos >= dur && !ctrl.value.isPlaying) {
      if (_loopCount < 2) {
        _loopCount++;
        _seekPending = true;
        ctrl.seekTo(Duration.zero).then((_) { _seekPending = false; ctrl.play(); });
      } else {
        _loopCount = 0;
      }
    }
  ...
  void _seekBy(int seconds) {
    final ctrl = widget.controller;
    final next = ctrl.value.position + Duration(seconds: seconds);
    final clamped = next < Duration.zero ? Duration.zero : (next > ctrl.value.duration ? ctrl.value.duration : next);
    ctrl.seekTo(clamped);
    _revealControls();
    if (seconds < 0) {
  ...
  Widget build(BuildContext context) {
    ...
    final progress    = duration.inMilliseconds > 0
        ? (ctrl.value.position.inMilliseconds / duration.inMilliseconds).clamp(0.0, 1.0)
        : 0.0;
    final t = AppThemeNotifier.of(context);
    ...
    return GestureDetector(
      ...
      child: Stack(
        ...
        children: [
          ...
          // Buffering spinner (only when not showing controls)
          if (isBuffering && !_showControls)
            SizedBox(
              width: 36, height: 36,
              ...
              children: [
                Expanded(
                  child: GestureDetector(
                    ...
                    child: _SeekFlashOverlay(
                        visible: _showSeekLeft, isLeft: true),
                  ),
                ),
                Expanded(
                  child: GestureDetector(
                    ...
                    onDoubleTap: () => _seekBy(10),
                    child: _SeekFlashOverlay(
                        visible: _showSeekRight, isLeft: false),
                  ),
                ),
              ],
            ),
            ...
            child: IgnorePointer(
              ...
              child: Container(
                ...
                child: Column(
                  children: [
                    const Spacer(),
                    // Centre: -10s · play/pause · +10s
                    Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        _PlayerBtn(icon: Icons.replay_10, size: 32,
                            onTap: () => _seekBy(-10)),
                        ...
                        const SizedBox(width: 28),
                        _PlayerBtn(icon: Icons.forward_10, size: 32,
                            onTap: () => _seekBy(10)),
                      ],
                    ),
                    const Spacer(),
                    // Progress + time + fullscreen
                    Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      Row(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          _PlayerBtn(icon: Icons.replay_10, size: 36,
                              onTap: () => _seekBy(-10)),
                          ...
                          const SizedBox(width: 36),
                          _PlayerBtn(icon: Icons.forward_10, size: 36,
                              onTap: () => _seekBy(10)),
                        ],
                      ),
                      const Spacer(),
...
// ─── Вспомогательные виджеты плеера ──────────────────────────────────────────
...
class _SeekFlashOverlay extends StatelessWidget {
  final bool visible;
  final bool isLeft;
  const _SeekFlashOverlay({required this.visible, required this.isLeft});
      ...
      child: Container(
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.horizontal(
            left:  isLeft  ? const Radius.circular(80) : Radius.zero,
            right: !isLeft ? const Radius.circular(80) : Radius.zero,
          ),
        ),
        child: Center(
          child: Column(
