              ...
              child: AspectRatio(
                ...
                child: _videoError
                    ...
                    : _videoReady && _videoCtrl != null
                        ? _FavVideoPlayer(controller: _videoCtrl!)
                        : Container(
                            color: const Color(0xFF1a2330),
                            child: Center(
                              child: Builder(builder: (bCtx) => CircularProgressIndicator(
                                  color: AppThemeNotifier.of(bCtx).primary, strokeWidth: 2)),
                            ),
                          ),
              ),
            ),
            ...
            ...screenshots.map((url) => Padding(
              ...
              child: Container(
                ...
                child: AspectRatio(
                  aspectRatio: 16 / 9,
                  child: GestureDetector(
                    onTap: () => _videoCtrl?.pause(),
                    child: InteractiveViewer(
                    minScale: 1.0,
                    maxScale: 4.0,
                    onInteractionStart: (_) => _videoCtrl?.pause(),
                    child: CachedNetworkImage(
                      ...
                      placeholder: (ctx, url) => Container(
                        ...
                        child: Center(
                          child: Builder(builder: (bCtx) => CircularProgressIndicator(
                            color: AppThemeNotifier.of(bCtx).primary,
                            strokeWidth: 2,
                          )),
                        ),
                      ),
...
// ─── YouTube-style video player (избранное) ───────────────────────────────────

class _FavVideoPlayer extends StatefulWidget {
  final VideoPlayerController controller;
  const _FavVideoPlayer({required this.controller});

  @override
  State<_FavVideoPlayer> createState() => _FavVideoPlayerState();
}

class _FavVideoPlayerState extends State<_FavVideoPlayer> {
  ...
  void _seekBy(int seconds) {
    ...
    ctrl.seekTo(next < Duration.zero ? Duration.zero : (next > dur ? dur : next));
    _revealControls();
    if (seconds < 0) {
              ...
              children: [
                Expanded(
                  child: GestureDetector(
                    ...
                    child: _FavSeekFlash(visible: _showSeekLeft, isLeft: true),
                  ),
                ),
                Expanded(
                  child: GestureDetector(
                    behavior: HitTestBehavior.translucent,
                    onDoubleTap: () => _seekBy(10),
                    child: _FavSeekFlash(visible: _showSeekRight, isLeft: false),
                  ),
                ),
              ],
            ),
                ...
                child: Column(
                  children: [
                    ...
                    Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        _FavPlayerBtn(icon: Icons.replay_10, size: 32,
                            onTap: () => _seekBy(-10)),
                        ...
                        const SizedBox(width: 28),
                        _FavPlayerBtn(icon: Icons.forward_10, size: 32,
                            onTap: () => _seekBy(10)),
                      ],
                    ),
                    const Spacer(),
                              ...
                              children: [
                                ...
                                GestureDetector(
                                  onTap: () => Navigator.push(
                                    context,
                                    MaterialPageRoute(
                                      builder: (_) => _FavFullscreenScreen(
                                          controller: ctrl),
                                    ),
                                  ),
...
// ─── Полноэкранный плеер (избранное) ─────────────────────────────────────────

class _FavFullscreenScreen extends StatefulWidget {
  final VideoPlayerController controller;
  const _FavFullscreenScreen({required this.controller});

  @override
  State<_FavFullscreenScreen> createState() => _FavFullscreenScreenState();
}

class _FavFullscreenScreenState extends State<_FavFullscreenScreen> {
  bool   _showControls = false;
  Timer? _hideTimer;
  bool   _isDragging   = false;
  double _dragValue    = 0;
                  ...
                  child: Column(
                    children: [
                      ...
                      Row(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          _FavPlayerBtn(icon: Icons.replay_10, size: 36,
                              onTap: () => _seekBy(-10)),
                          ...
                          const SizedBox(width: 36),
                          _FavPlayerBtn(icon: Icons.forward_10, size: 36,
                              onTap: () => _seekBy(10)),
                        ],
                      ),
                      const Spacer(),
...
// ─── Вспомогательные виджеты плеера (избранное) ───────────────────────────────

class _FavPlayerBtn extends StatelessWidget {
  final IconData icon;
  final double   size;
...
class _FavSeekFlash extends StatelessWidget {
  final bool visible;
  final bool isLeft;
  const _FavSeekFlash({required this.visible, required this.isLeft});
